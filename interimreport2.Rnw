...
The prioritized ethnicities for cases are:  European; Maori; Pacific Peoples; Asian; Middle Eastern/Latin American/African (MELAA); Other Ethnicity; Residual Categories, though for the analyses in this report only the first five are used, as these categories cover the overwhelming number of cases, with only 1.9\% (22/1137) of cases having no Prioritized Ethnicity (Table~\ref{table:case}). 
<<echo=FALSE,include=FALSE,results=hide>>=
...
The distribution of the cases per category used are in Figure~\fig{fig:histcasecat}.
\begin{figure}[h!]
\begin{center}
<<fig=TRUE,echo=FALSE,width = 11>>=
par(cex.axis=2)
