...
\begin{center}
<<fig=TRUE,echo=FALSE,width = 15,height=12>>=
ggplot(tp, aes(merge, cases, fill=Ethnicity)) + 
+     geom_bar(stat="identity", position="dodge")+
+     # theme(text=element_text(size=45))+
...
\begin{center}
<<fig=TRUE,echo=FALSE,width = 15,height=12>>=
tpsub$order <- factor(tpsub$merge, as.character(tpsub$merge))
ggplot(tpsub, aes(x=order, cases, fill=Ethnicity)) +
     geom_bar(stat="identity", position="dodge")+
     # theme(text=element_text(size=45))+
...
\begin{center}
<<fig=TRUE,echo=FALSE,width = 15,height=12>>=
ggplot(tpsub, aes(x=order, perCap, fill=Ethnicity)) +
     geom_bar(stat="identity", position="dodge")+
     # theme(text=element_text(size=45))+
...
tpsub1$order <- factor(tpsub1$merge, as.character(tpsub1$merge))
ggplot(tpsub1, aes(order, cases, fill=Ethnicity)) +
     geom_bar(stat="identity", position="dodge")+
     # theme(text=element_text(size=45))+
     ylab("cases")+theme(axis.text.x = element_text(angle = 90, hjust = 0))
@
...
\begin{center}
<<fig=TRUE,echo=FALSE,width = 15,height=12>>=
ggplot(tpsub1, aes(order, perCap, fill=Ethnicity)) +
     geom_bar(stat="identity", position="dodge")+
     # theme(text=element_text(size=45))+
...
\begin{table}[hbtp]
\footnotesize
\caption{proportions of the naive population the wuld be covered if vaccinated by age groups}
<<results=tex,echo=FALSE>>=
# This chunk uses "results=tex"
library(Hmisc)
