...
\end{center}
\caption{New Zealand population by age and estimated numbers of na\"{\i}ve people in each age class using National Immunisation Register level data and national census data as the denominator. Note no values should be greater than 1 (100\% vaccine cover), and the maximum discordance is > 1600\%.}
