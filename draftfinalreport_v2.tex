...
\hline
\end{tabular}\end{center}\caption{Measles cases (Cases), population sizes (Population) and per capita rates per 10,000 (Per capita) of measles in specific age (Age), ethnicity (Ethnicity) and socio-economic deprivation (NZDep) categories from 2007-2014}
\label{table:percap}
\end{table}
...
\hline
\end{tabular}\end{center}\caption{Proportions of the na\"{\i}ve population the would be covered if vaccinated by age groups}
...
(Intercept) & -6.1644 & 0.1267 & -48.65 & 0.0000 \\ 
  ...
  Age25+:EthnicityPacific & -0.6629 & 0.4279 & -1.55 & 0.1296 \\ 
   \hline
\end{tabular}\caption{Summary of the regression model results, estimated using \autoref{eq:reg} and a quasipoisson error structure, for measles risk factors for cases from 2007-2014}
...
\hline
\end{tabular}\end{center}\caption{Number of cases, length of hospital stay, cost (NZ\$), cost per case (NZ\$) and cost per day (NZ\$) for patients with measles as the primary diagnosis, by year and gender, 2000--2014. Note "Length of stay" is number of complete days spent in hospital, thus 0 is no nights spent in hospital (see \autoref{table:regression_costs})}
\label{table:cases}
 \centering
 \begin{tablenotes}
      \small
