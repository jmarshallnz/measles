...
\item The mean cost of those measles case that attend hospital is estimated to be \$1,877 per attending case. Approximately 17\% of measles cases attend hospital.
...
colnames(pop)<-0:100
pop<-t(pop)
impop<-c(popimmune$Immunity[1:6],rep(popimmune$Immunity[7],8),rep(popimmune$Immunity[8],5),rep(popimmune$Immunity[9],5), rep(popimmune$Immunity[10],9),rep(popimmune$Immunity[11],20),rep(popimmune$Immunity[12],48))
...
teth<-revalue(test$EthnicityPrioritised, c("European or Other"="European", "Middle Eastern/Latin American/African"="MLA","Pacific Peoples"="Pacific","Response cannot be classified"="None","Unknown"="None"));
...
tpsub$Ethnicity<- relevel(tpsub$Ethnicity, "European")
...
yr_range<-c("2-5","5-11","5-16","5-18","11-16","11-18","0-29","0-44",">8")
...
totdpdf<-read.csv("2013mapdata_tot.csv",header=T)
...
nzdpdf<-read.csv("2013mapdata_nzers.csv",header=T)
# output top countries
...
imdpdf<-read.csv("2013mapdata_immigration.csv",header=T)
# output top countries
...
t12006<-read.csv("MMR1summary2006.txt",header=T,sep="")
t22006<-read.csv("MMR2summary2006.txt",header=T,sep="")
t12007<-read.csv("MMR1summary2007.txt",header=T,sep="")
t22007<-read.csv("MMR2summary2007.txt",header=T,sep="")
t12008<-read.csv("MMR1summary2008.txt",header=T,sep="")
t22008<-read.csv("MMR2summary2008.txt",header=T,sep="")
t12009<-read.csv("MMR1summary2009.txt",header=T,sep="")
t22009<-read.csv("MMR2summary2009.txt",header=T,sep="")
t12010<-read.csv("MMR1summary2010.txt",header=T,sep="")
t22010<-read.csv("MMR2summary2010.txt",header=T,sep="")
t12011<-read.csv("MMR1summary2011.txt",header=T,sep="")
t22011<-read.csv("MMR2summary2011.txt",header=T,sep="")
t12012<-read.csv("MMR1summary2012.txt",header=T,sep="")
t22012<-read.csv("MMR2summary2012.txt",header=T,sep="")
t12013<-read.csv("MMR1summary2013.txt",header=T,sep="")
t22013<-read.csv("MMR2summary2013.txt",header=T,sep="")
t12014<-read.csv("MMR1summary2014.txt",header=T,sep="")
t22014<-read.csv("MMR2summary2014.txt",header=T,sep="")
...
t12014,
t22014)
colnames(table_nir_mmr)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
yr_nir<-c('MMR1 2006',
'MMR2 2006',
'MMR1 2007',
'MMR2 2007',
...
"MMR1 2014",
"MMR2 2014")
table_nir_mmr$Year <-yr_nir
table_nir_mmr<-table_nir_mmr[c(10,1:7)]
table_nir_mmr[2:7]<-table_nir_mmr[2:7]*100
latex(table_nir_mmr, file="", table.env=FALSE,rowname=NULL)
...
@
\caption{Measles cases (Cases), population sizes (Population) and per capita rates per 10,000 (Per capita) of measles in specific age (Age), ethnicity (Ethnicity) and socio-economic deprivation (NZDep) categories from 2007-2014. MLA is Middle Eastern/Latin American/African}
\label{table:percap}
\end{table}
...
\begin{align*} \label{eq:reg}
 ...
 \log(\lambda) &= \alpha + \beta _a (x_a)+ \beta _e(x_e)+ \beta _s (x_s) + \beta _a{}_e(x_a \times x_e)
  \end{align*}
Where $y$ is the number of cases, $\lambda$ is cases per capita, $n$ is population size, $\alpha$ is the intercept, $_a$ signifies age, $_e$ signifies Prioritized Ethnicity and $_s$ signifies NZDep.
...
@
\caption{Significance of different predictor variables for measles risk factors from 2007-2014, estimated using \autoref{eq:reg} and a quasipoisson error structure}
\label{table:anova}
\end{table}
...
@
\caption{Summary of the regression model results using \autoref{eq:reg} and a quasipoisson error structure, for measles risk factors for cases from 2007-12 June 2014}
...
\end{center}
\caption{Age of vaccination of vaccinated measles cases, 2007--12~June~2014. Recommended vaccination age in months is shown for the measles, mumps, rubella (MMR) dose 1 (12 months) and MMR dose 2 (4 years)}
\label{fig:vaccstat}
\end{figure}
...
latex(table_nir_mmr, file="", table.env=FALSE,rowname=NULL)
@
\caption{Summary of the proportion of 2006--2014 birth cohorts vaccinated against measles-mumps-rubella (MMR) dose 1 and MMR2 across area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12006}, \autoref{fig:fig22006}, \autoref{fig:fig12007}, \autoref{fig:fig22007}, \autoref{fig:fig12008}, \autoref{fig:fig22008}, \autoref{fig:fig12009}, \autoref{fig:fig22009}, \autoref{fig:fig12010}, \autoref{fig:fig22010}, \autoref{fig:fig12011}, \autoref{fig:fig22011}, \autoref{fig:fig12012}, \autoref{fig:fig22012}, \autoref{fig:fig12013}, \autoref{fig:fig22013}, \autoref{fig:fig12014}, and \autoref{fig:fig22014}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22006)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22006[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2006 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22006}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12007)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12007[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2007 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12007}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22007)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22007[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2007 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22007}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12008)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12008[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2008 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12008}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22008)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22008[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2008 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22008}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12009)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12009[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2009 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12009}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22009)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22009[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2009 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22009}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12010)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12010[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2010 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12010}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22010)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22010[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2010 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22010}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12011)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12011[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2011 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12011}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22011)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22011[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2011 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22011}.}
...
\begin{center}
 ...
 \end{center}
    \caption{Proportion of the New Zealand population born in 2011 vaccinated against measles using MMR2 in each area unit. Note no adjustment is made for vaccine efficacy. The histogram shows the frequency of the proportion vaccinated across the country's area units with 95\% shown in red and the median for the cohort shown in orange. See \autoref{table:tab_mmr_nir} and the supplementary data for details.}
%\end{center}
\label{fig:fig22011}
\end{sidewaysfigure}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12012)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12012[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2012 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12012}.}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22012)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22012[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2012 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22012}.}
...
\begin{center}
 ...
 \end{center}
    \caption{Proportion of the New Zealand population born in 2012 vaccinated against measles using MMR2 in each area unit. Note no adjustment is made for vaccine efficacy. The histogram shows the frequency of the proportion vaccinated across the country's area units with 95\% shown in red and the median for the cohort shown in orange. See \autoref{table:tab_mmr_nir} and the supplementary data for details.}
% \end{center}
\label{fig:fig22012}
\end{sidewaysfigure}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12013)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12013[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2006 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12013}.}
...
\begin{center}
 ...
 \end{center}
    \caption{Proportion of the New Zealand population born in 2013 vaccinated against measles using MMR1 in each area unit. Note no adjustment is made for vaccine efficacy. The histogram shows the frequency of the proportion vaccinated across the country's area units with 95\% shown in red and the median for the cohort shown in orange. See \autoref{table:tab_mmr_nir} and the supplementary data for details.}
% \end{center}
\label{fig:fig12013}
\end{sidewaysfigure}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22013)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22013[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2006 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22013}.}
...
\end{center}
\caption{Proportion of the New Zealand population born in 2013 vaccinated against measles using MMR2 in each area unit. Note no adjustment is made for vaccine efficacy. The histogram shows the frequency of the proportion vaccinated across the country's area units with 95\% shown in red and the median for the cohort shown in orange. See \autoref{table:tab_mmr_nir} and the supplementary data for details.}
%     \end{center}
\label{fig:fig22013}
\end{sidewaysfigure}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t12014)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t12014[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2014 birth cohort vaccinated against MMR1 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12014}.}
...
\begin{center}
 ...
 \end{center}
    \caption{Proportion of the New Zealand population born in 2014 vaccinated against measles using MMR1 in each area unit. Note no adjustment is made for vaccine efficacy. The histogram shows the frequency of the proportion vaccinated across the country's area units with 95\% shown in red and the median for the cohort shown in orange. See \autoref{table:tab_mmr_nir} and the supplementary data for details.}
% \end{center}
\label{fig:fig12014}
\end{sidewaysfigure}
...
% \begin{table}
% <<include=FALSE,echo=FALSE,results=tex>>=
% colnames(t22014)<-c("Min","1st Q","Median","Mean","3rd Q","Max","NA")
% latex(t22014[1:7], file="", table.env=FALSE,rowname=NULL)
% @
% \caption{Summary of the proportion of 2014 birth cohort vaccinated against MMR2 across the area units. Area units are aggregations of meshblock data. See \autoref{fig:fig22014}.}
...
<<echo=FALSE,include=FALSE,results=hide>>=
MMR1<-c(0.9604,0.9621,0.971,0.9722,0.9667,0.9643,0.9667,0.6333,0)
MMR2<-c(0.9167,0.9099,0.9048,0.8889,0.7059,0.0345,0.0041,0,0)
res_bar<-cbind(MMR1,MMR2)
rownames(res_bar)<-c("2006","2007","2008","2009","2010","2011","2012","2013","2014")
@

\begin{figure}
\begin{center}
<<fig=TRUE,echo=FALSE>>=
barplot(t(res_bar),beside=T,ylim=c(0,1),xlab="Birth year",ylab="Proportion vaccinated",legend=colnames(res_bar),
        args.legend = list(x = "topright",bty="n",inset=c(0.1,0.1)))
abline(h=0.90,col="red",lty=3,lwd=4)
abline(h=0.95,col="orange",lty=3,lwd=4)
@
\end{center}
...
The correlation between the percentage vaccinated against MMR1 and MMR2 across the area units is shown in figure \autoref{fig:cor_nir}.
...
@
\caption{Highest measles incidence per million (2013)}
...
@
\caption{Lowest national measles vaccine cover (\%), 2013}
\label{table:cover12}
\end{table}
...
@
\caption{Non-New Zealander travel and immigration numbers, 0--19 year olds, 2013}
...
@
\caption{New Zealander travel numbers by destination, 2013}
...
@
\caption{Total New Zealand traveller numbers by country (New Zealand nationals and 0--19 year old immigrants, 2013)}
...
@
\caption{Risk of measles importation to New Zealand in 2013, estimated by 0--19 year old international and all New Zealander traveller numbers multiplied by measles incidence in the source or destination location, see also \autoref{table:nzrisk12} and \autoref{table:imrisk12}}
\label{table:risk12}
\end{table}
...
@
\caption{Risk of measles importation to New Zealand due to New Zealander travel in 2013, estimated by all traveller numbers multiplied by measles incidence in the source or destination location, see also \autoref{table:imrisk12} and \autoref{table:risk12}}
...
@
\caption{Risk of measles importation to New Zealand due to non-New Zealander 0--19 year old travel and immigration in 2013, estimated by traveller numbers multiplied by measles incidence in the source or destination location, see also \autoref{table:nzrisk12} and \autoref{table:risk12}}
...
The additional data we received from the Ministry from the NIR database allows us to look at vaccination coverage as a finer scale. There is consistently high vaccination cover by MMR1 (often exceeding 95\%, see \autoref{fig:mmr_nir}), with lower rates for MMR2, approaching 90\% (\autoref{fig:mmr_nir}). The heterogeneity discussed above, however, does not appear to be the same for each area unit, as there is less correlation among the percent coverage for the two vaccines (\autoref{fig:cor_nir}) than might be expected, even at a yearly level (not shown).
...
\item Distribution of numbers of na\"{\i}ve among New Zealand is uneven, with the majority in DHBs with larger urban areas, though the \%  na\"{\i}ve is similar across DHBs.
...
\item The reproduction number for measles in the partially immune New Zealand population is often close to and exceeding one, suggesting increased population level immunity is required to ensure prevention of measles persistence following importation.
 \item Additional vaccination of the currently na\"{\i}ve population in New Zealand required to push $R_v$ below one ranges from 17\% to 34\% among DHBs, and is 28\% at the national level (approximately 128,034 vaccinations).
\end{itemize}
...
@
\caption{Reported direct costs in NZ\$, with the number of cases (Cases), length of hospital stay (Days), total cost (Cost, NZ\$), cost per case (NZ\$) and cost per day (NZ\$) for patients with measles as the primary diagnosis, 2000--2014}
\label{table:hosp}
 \centering
 \begin{tablenotes}
      \small
...
@
\caption{Number of cases, length of hospital stay, cost (NZ\$), cost per case (NZ\$) and cost per day (NZ\$) for patients with measles as the primary diagnosis, by year and gender, 2000--2014. Note \textit{Length of stay} is number of complete days spent in hospital, thus 0 is no nights spent in hospital}
\label{table:cases}
 \centering
 \begin{tablenotes}
      \small
...
Wages lost for the period January~2008--August~2014 for the 247 cases and care givers was estimated to be \$207,155. This consisted of \$104,539 for the cases and \$102,616 for the care giver, but did not include wage losses for cases under 15 years of age. Overall, the cost per case from 2008--2014 was estimated to be \$2923.27 (\$839 in forgone wages, \$1,765 in management costs, and \$1,877 in hospital costs for the 17\% of cases hospitalised). This figure brings an approximate estimate of \$546650.78 for 187 cases for the current outbreak in 2014 alone, which is comprised of earnings lost, case management and hospitalisation costs.
...
@
\caption{DHB and national level catch up vaccination rates and estimated outbreak sizes post catch-up vaccination. \textit{Size}: DHB Population, Statistics NZ 2013;    
\textit{Na\"{\i}ve}:	DHB na\"{\i}ve population $\left(x_0\times\text{Size}\right)$;		
...
\begin{sidewaystable}\small
<<results=tex,echo=FALSE>>=
colnames(costtable20)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C")
...
\begin{sidewaystable}\small
<<results=tex,echo=FALSE>>=
colnames(costtable50)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C")
...
\item The mean cost for measles cases that attend hospital is estimated to be \$1,877.
